...
class Real_Weak_Classifier_1(Weak_Classifier):
    def __init__(self, id, plus_rects, minus_rects, num_bins):
        super().__init__(id, plus_rects, minus_rects, num_bins)
    ...
    def calc_error(self, weights, labels):
        ...
        ######## TODO ########
        ######################
        data_max = np.max(self.activations)
        data_min = np.min(self.activations)
        self.thresholds = data_min * np.ones((self.num_bins,))
        self.bin_pqs = np.zeros((2, self.num_bins))
        step = (data_max-data_min)/self.num_bins
        for i in range(self.num_bins):
            self.thresholds[i] += step * (i+1)
        pos_m = np.where(labels == 1, 1, 0)
        neg_m = np.where(labels == -1, 1, 0)
        for i in range(self.num_bins):
            if i == 0:
                idx_bin_1 = np.where(self.activations < self.thresholds[i], 1, 0)
                #idx_bin_2 = np.where(self.activations < self.thresholds[i + 1], 1, 0)
            else:
                idx_bin_1 = np.where(self.activations>self.thresholds[i-1],1,0)
                idx_bin_2 = np.where(self.activations < self.thresholds[i],1, 0)#### index i+1????
                idx_bin_1 = np.multiply(idx_bin_1,idx_bin_2)
            indicator = np.multiply(pos_m,idx_bin_1)
            self.bin_pqs[0, i] = np.sum(np.multiply(weights, indicator))
            indicator = np.multiply(neg_m, idx_bin_1)
            self.bin_pqs[1, i] = np.sum(np.multiply(weights, indicator))
            if self.bin_pqs[1, i] == 0:
                self.bin_pqs[1, i] = 0.0001
...
class Real_Weak_Classifier_2(Weak_Classifier):
    def __init__(self, id, plus_rects, minus_rects, num_bins):
        super().__init__(id, plus_rects, minus_rects, num_bins)
        self.thresholds = None  # this is different from threshold in ada_weak_classifier, think about it
        self.bin_pqs = None
